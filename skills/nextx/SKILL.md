...
3. When the user asks to update or upgrade NextX, run `<nextx> upgrade` (use `--dry-run` for a preview). The command reuses the canonical installer, preserves the configured Vault, reinstalls the isolated runtime, and refreshes detected Agent Skills. For a source checkout, ask the user to pull the intended branch/tag first; never silently fetch or switch branches from the conversation.
4. If an `agent_skills` entry reports `conflict`, do not overwrite it silently. Explain its path and offer the explicit installer option `--force-agent-skills`. If it reports `not_detected`, do not create a guessed Agent home; the user may explicitly request `--agents all`.
5. After installation or upgrade, run `<nextx> next-step`. It is read-only and returns `setup_required`, `self_required`, `growth_required`, or `ready` plus the next safe action. Use it at the beginning of a setup conversation and after completing setup or Self configuration.
6. Treat “初始化 NextX” or “开始使用 NextX” as explicit authorization to run `<nextx> setup --runtime RUNTIME --yes` (omit runtime when absent). Default to `~/Documents/NextX`; use another Vault only when the user names it. Then run `<nextx> next-step`; do not claim setup is ready based only on a directory existing.
7. If Self configuration is needed, collect these user-authored values in one compact dialogue: positioning, audience, stage, 3–4 pillars, boundaries, and 1–10 authentic voice samples. For Growth Loop, also collect a user-confirmed current stage (`launch` / `ramp` / `steady`), one weekly objective, target reader, profile promise, CTA, weekly focus and lane allocation. Never invent them. You may propose a simple default allocation from the declared stage, but the user must confirm it. Query `<nextx> contracts --name self`, then send the confirmed JSON through `<nextx> configure-self --input-json -`.
...
| “更新 / 升级 NextX” | Run `<nextx> upgrade`; report the installer result and ask for a new Agent session if the host cached the old Skill. |
